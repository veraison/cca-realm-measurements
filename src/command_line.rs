...
pub struct Args {
    ...
    /// Input DTB template. Before measurement, the DTB is patched to be
    /// compatible with the given VMM parameters. This is experimental.
    /// It is unlikely that the DTB resulting from patching matches exactly
    /// that generated by the VMM.
    #[arg(long, value_name = "file")]
    pub dtb_template: Option<String>,
    ...
    /// Input DTB. The DTB is measured as-is, not modified. It is therefore only
    /// valid for one set of VMM parameters. Changes to the VMM parameters are
    /// likely to affect the DTB.
    #[arg(long, value_name = "file")]
...
/// Test whether the given arguments are valid. Return an error in case of
/// inconsistency.
#[allow(dead_code)]
pub fn check_args(args: &Args) -> Result<()> {
    if args.dtb.is_some() && args.dtb_template.is_some() {
        bail!("either --dtb or --dtb-template can be set but not both");
    }
    Ok(())
}
